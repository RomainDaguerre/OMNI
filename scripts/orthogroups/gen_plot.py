...
RCODE = (
    ...
    'my_species_colors <- c("#0072B2", "#D55E00", "#999999")  # Genomes = bleu, Transcriptomes = orange, None = gris\n'
    "# Bar height ratio\n"
    "my_bar_height <- 0.7\n"
    "\n"
    "# Legend\n"
    ...
    "# Define if species are genomes or transcriptomes\n"
    "my_species_type <- c%s6\n"
    "my_species_type <- my_species_type[seq(1, length(my_species_type), by = 4)]\n"
    "my_species_type <- factor(my_species_type)\n"
    "my_species_type <- factor(my_species_type, levels=c('genome', 'transcriptome', 'NULL'))"
    "\n"
    "my_percentage <- c%s3\n"
    "my_values <- c%s4\n"
    "\n"
    ...
    "df = data.frame(my_species,my_percentage,my_values,category,my_species_type)\n"
    "\n"
    "figure <- ggplot() + \n"
    "  \n"
    ...
    '  ylab("\n%BUSCOs") + \n'
    "\n"
    "  theme(plot.title = element_text(family=my_family, hjust=0.5, colour = 'black', size = rel(3.2)*my_size_ratio, face = 'bold')) + \n"
    "  theme(legend.position=\"top\",legend.title = element_blank()) + \n"
    "  theme(legend.text = element_text(family=my_family, size = rel(2.2)*my_size_ratio)) + \n"
    '  theme(panel.background = element_rect(color="#FFFFFF", fill="white")) + \n'
    "  theme(panel.grid.minor = element_blank()) + \n"
    "  theme(panel.grid.major = element_blank()) + \n"
    "  theme(axis.text.y = element_text(family=my_family, colour = my_species_colors[as.numeric(my_species_type) ], size = rel(3)*my_size_ratio, face = 'italic')) + \n"
    "  theme(axis.text.x = element_text(family=my_family, colour = 'black', size = rel(2.66)*my_size_ratio)) + \n"
    ...
    'sep=""), \n'
    '             y=3, x = i, size = labsize*7*my_size_ratio, colour = "black", hjust=0, family=my_family)\n'
    "  }\n"
    "  \n"
...
'''modif'''
def load_species_types(csv_file):
    """
    Load species types from the given CSV file using the 'Type' column.
    :param csv_file: Path to the CSV file containing species and their types.
    :return: Dictionary mapping species names to their types.
    """
    species_types = {}
    with open(csv_file, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            species_type = row.get("Type")
            if species and species_type:
                species_types[species.strip()] = species_type.strip()
    return species_types

def get_species_types(species_list, species_types):
    """
    Retrieve the types corresponding to the given species list.
    :param species_list: List of species names.
    :param species_types: Dictionary mapping species names to types.
    :return: Tuple of species types.
    """
    return tuple(species_types.get(species.replace("_"," "), "NULL") for species in species_list)

def get_species_no_type(species_list):
    """
    Return a tuple of None for each species in the species list.
    :param species_list: List of species names.
    :return: Tuple of None values.
    """
    return tuple("NULL" for _ in species_list)

def _write_r_code(data, csv_file, type_species):
    """
    ...
    if type_species == True :
        species_types_dict = load_species_types(csv_file)
        species_types_tuple = get_species_types(sorted(data["species"], reverse=True), species_types_dict)
    else : 
        species_types_tuple = get_species_no_type(sorted(data["species"], reverse=True))
    ...
    r_file.write(
        ...
        .replace("%s5", "(1)")
        .replace("%s6", str(species_types_tuple))
    )
''''''
...
def _set_args():
    ...
    optional.add_argument(
        ...
        required=True,
        dest="csv_file",
        help="Path to the type_proteome CSV file",
    )
    optional.add_argument(
        "-t",
        "--type",
        help="To stop using the type method for coloring species",
        dest="type",
        action="store_true"
    )
    optional.add_argument(
...
def main():
    ...
    try:
        ...
        csv_file = _csv_file
        type_species = _type
        _write_r_code(data, csv_file, type_species)
